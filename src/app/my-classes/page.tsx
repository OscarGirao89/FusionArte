...
const teacherName = 'Elena Garcia'; // Hardcoded for teacher role simulation
                    ...
                    {upcomingClasses.map(c => (
                         <Card key={c.id} className="flex flex-col">
                            <CardHeader>
                                ...
                                <Badge variant="secondary" className="w-fit">{c.level}</Badge>
                            </CardHeader>
                    ...
                    {myClasses.map(c => (
                        <li key={c.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border rounded-lg">
                            <div className="mb-4 sm:mb-0">
                                <p className="font-bold">{c.name} - {c.level}</p>
                                <p className="text-sm text-muted-foreground">{c.day} a las {c.time} en {c.room}</p>
                            </div>
                            <div className="flex gap-2">
                                <Button variant="secondary" size="sm"><Users className="mr-2 h-4 w-4" /> Alumnos</Button>
                                <Button variant="outline" size="sm"><BookCheck className="mr-2 h-4 w-4" /> Pasar Lista</Button>
                            </div>
                        </li>
                    ))}
                </ul>
